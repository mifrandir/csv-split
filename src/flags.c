...
static size_t parse_flag_by_index(struct Config *cfg,
                                  const int argc,
                                  char **argv,
                                  size_t *at,
                                  size_t flag) {
  ...
  switch (flag) {
    ...
    case LINE_COUNT:
      ...
      return *at += 2;  // Read extra argument.
    case DELIMITER:
      if (arg_at == argc || strlen(argv[arg_at]) > 1) {  // Expected single character.
    ...
    case REMOVE_COLUMNS:
      ...
      return *at += 2;
    case INCLUDE_REMAINDERS:
      cfg->include_remainders = 1;
      return (*at)++;
    case HELP:
      print_help();
      exit(0);
  ...
  do {
    ...
    for (struct Flag *f = FLAGS; f < FLAGS + FLAG_COUNT; f++) {
      if (argv[start][subflag] == f->short_id) {
        ...
        if (tmp_at > start + 1) {  // Boolean flags are not allowed after flags
...
//
// Exits if a PARSE_ERR is encountered.
static size_t parse_long_flag(struct Config *cfg, const int argc, char **argv, size_t *at) {
  for (struct Flag *f = FLAGS; f < FLAGS + FLAG_COUNT; f++) {
