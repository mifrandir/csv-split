...
size_t strsub(char *str, const char old, const char new) {
  size_t count = 0;
...
size_t remove_linebreak(char *str, size_t len) {
...
void initialise_batch(
  ...
  if (batch->lines == NULL) {
    ...
    for (size_t *p = batch->line_lengths; p - batch->line_lengths < max_lines;
         p++) {
      *p = 0;
    }
  }
...
void split_csv(const struct Config *cfg) {
  ...
  do {
    ...
    process_batch(
        cfg, f, input_length, include_column, output_headers, &output_buffer);
