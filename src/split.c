...
static void load_headers(const struct Config *cfg, FILE *f, size_t *len, char ***headers) {
...
static size_t find_relevant_columns(
		const struct Config *config, const size_t input_length, char **headers, char *include_column) {
...
static void load_values(
	...
	if (input_column_count !=
		...
		fprintf(
				stderr, "Unexpected number of columns in input. (expected: %lu, found: %lu)\n",
				input_column_count, actual_column_count);
		exit(1);
	}
...
static void write_batch(const struct Config *config, char **headers, struct Batch *output) {
...
static void process_batch(
	...
	for (i = 0; i < config->line_count; i++) {
		...
		filter_line(
				config, input_column_count, value_buffer, include_column, output->column_count,
				output->values[i]);
	}
	output->line_count = i;
	write_batch(config, headers, output);
...
static void initialise_batch(
		const struct Config *config, struct Batch *batch, const size_t max_lines, const size_t cols) {
	if (batch->lines == NULL) {
...
void split_csv(const struct Config *cfg) {
	...
	size_t output_length = find_relevant_columns(cfg, input_length, input_headers, include_column);
	LOG("input_length=%lu, output_length=%lu\n", input_length, output_length);
	char *output_headers[output_length];
	filter_line(cfg, input_length, input_headers, include_column, output_length, output_headers);
