...
static size_t read_file_names(const char *path, char **buffer) {
...
static int compare_dir_by_list(
    const char *dir_a, const char *dir_b, size_t num_files, char **file_names) {
  ...
  for (size_t i = 0; i < num_files; i++) {
    ...
    if (compare_files(path_a, path_b)) {
      printf(
          "Equality of directories could not be confirmed. Diverging files:\n%s\n%s\n",
          path_a,
          path_b);
      free(path_a);
      free(path_b);
...
int compare_directories(const char *dir_a, const char *dir_b) {
  ...
  size_t num_files_a = read_file_names(dir_a, file_names_a);
  LOG("Read %lu files from %s\n", num_files_a, dir_a);
  char *file_names_b[MAX_FILE_COUNT];
  size_t num_files_b = read_file_names(dir_b, file_names_b);
  LOG("Read %lu files from %s\n", num_files_b, dir_b);
