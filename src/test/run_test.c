...
// static size_t extend_directory(const char *dir, const char *sub, char
// *buffer) {
...
int run_test(struct Test *test) {
	...
	char *test_command = malloc(
			(128 + strlen(binary_path) + strlen(temp_dir_path) +
			 strlen(data_dir_path) + strlen(test->flags)) *
			sizeof(char));
	sprintf(
			test_command,
	...
	FILE *process = popen(
			test_command,
			"w");	 // We're opening in write mode so that processes
						 // stdout is automatically written to our stdout
	int err;
	if ((err = pclose(process))) {
		ERR_LOG(
				"Could not run command. Command: %s; Error Code: %d\n",
				test_command,
				err);
		printf("FAILED\n");
		return 1;
